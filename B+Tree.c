...
record * make_record(char value[20]);
node * make_node(void);
node * make_leaf(void);
...
node * insert(node * root, int key, char value[20]);
...
record * make_record(char value[20]) {
	...
	if (new_record == NULL) {
		...
		strcpy(new_record->value,value);//new_record->value = value;
	}
	return new_record;
}
...
node * insert(node * root, int key, char value[20]) {
    ...
    if (record_pointer != NULL) {
        ...
        strcpy(record_pointer->value,value);
        return root;
    }
...
int main(int argc, char ** argv) {
	...
	if (argc > 2) {
		...
		while (!feof(fp)) {
			fscanf(fp, "%d %s", &input_key,value);
			 //value[0]='\0';

			root = insert(root, input_key, value);
		}
		fclose(fp);
		print_tree(root);
        return EXIT_SUCCESS;
	...
	while (scanf("%c", &instruction) != EOF) {
		...
		case 'i':
            ...
            line_consumed = true;
            count = sscanf(buffer, "%d %s", &input_key, value);
            if (count == 1)
